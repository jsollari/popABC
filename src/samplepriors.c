...
*/
void choosetop(struct params *pm,int nevt,int ntop,int npop);
	...
	if(npop>1){
		...
		for(cevt=1 ; cevt<nevt ; ++cevt)
			if(pm->tev[cevt] <= pm->tev[cevt-1])
				printerr("times not monotonic increasing");
...
void choosetop(struct params *pm,int nevt,int ntop,int npop){
	...
	if((P_top.type==0 || P_top.type==1 ||P_top.type==3 || P_top.type==4) && npop < 6){
		...
		switch(i){
			...
			default: printerr("choosing branching sequence");
		}
	}
		...
		printerr("with more than 5 populations the topology as to be fixed using option 2 or 5");
}
